cheatsheet do
  ...
  category do
    ...
    entry do
      ...
      name "Save the search buffer by renaming it to a" \
           "unique new name."
      notes "Outside of the result buffer it's bound to `C-c s s`."
    end
    ...
    entry do
      ...
      name "Save the search buffer and interactively" \
    ...
    entry do
      ...
      name "Open the search manager."
      notes "This is available both in result buffer and globally bound to `C-c s l`."
    end

    entry do
      command "C-c s k"
      command "(rg-kill-saved-searches)"
      name "Kill all saved searches except for the one that matches `rg-buffer-name`"
    end
  end
end
